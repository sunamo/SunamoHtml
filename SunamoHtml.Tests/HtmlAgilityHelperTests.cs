...
public class HtmlAgilityHelperTests
{
    ...
    public async Task CreateHtmlDocumentTest()
    {
        ...
        var node = HtmlAgilityHelper.NodeWithAttr(htmlDocument.DocumentNode, true, "div", "class", "css-j7qwjs");
    }
    [Fact]
    public void PairsDdDtTest2()
    {
        ...
        var pairs = HtmlAgilityHelper.PairsDdDt(htmlDocument.DocumentNode, true, replacements);
        var accessories = pairs["Příslušenství:"];
        var area = pairs["Plocha:"];
    }
    [Fact]
	...
	<dd>
        ...
        htmlDocument.LoadHtml(input);
        Dictionary<string, string> replacements = new Dictionary<string, string>();
        replacements.Add("<span class=\"icon icon--check\">", "✓");
        var pairs = HtmlAgilityHelper.PairsDdDt(htmlDocument.DocumentNode, true, replacements);
    }
    [Fact]
    public async Task NodesWithAttrTest()
    {
        ...
        var c_products__list = HtmlAgilityHelper.NodeWithAttr(htmlDocument.DocumentNode, true, "div", "class", "c-products__list grid");
    ...
    {
        var htmlDocument = HtmlAgilityHelper.CreateHtmlDocument();
        //htmlDocument.Load(@"D:\_Test\PlatformIndependentNuGetPackages\SunamoHtml\a.html");
        htmlDocument.Load(@"E:\vs\Projects\_tests\CompareTwoFiles\CompareTwoFiles\html\2.html");
        var adsParent = htmlDocument.DocumentNode.FirstChild; //HtmlAgilityHelper.NodeWithAttr(hd.DocumentNode, true, "div", "class", "row inzlist");
